...
# -*- coding: utf-8 -*-
"""
add_games_to_louvain_graph.py

Purpose:
    Add the 50 games to the existing graph, run Louvain algorithm,
    and analyze which communities the games end up in.

Usage:
    python add_games_to_louvain_graph.py
"""
...
def add_games_to_graph(G: nx.Graph, games_df: pd.DataFrame, 
                      existing_communities: Dict[str, int],
                      similarity_threshold: float = 0.7) -> nx.Graph:
    """Add the 50 games to the graph by connecting them to existing nodes"""
    
    print(f"[INFO] Adding {len(games_df)} games to the graph")
    ...
    print(f"[INFO] Graph now has {G.number_of_nodes()} nodes")
    
    # For now, we'll add edges based on the cosine similarity results
    # We'll connect each game to its assigned community members
    edges_added = 0
    ...
    for _, row in games_df.iterrows():
        ...
        # Connect the game to some community members
        # We'll connect to a subset of community members based on similarity
        if similarity > -0.1:  # Only connect if similarity is reasonable
            # Connect to top-degree nodes in the community
            ...
            if community_nodes:
                ...
                # Connect to top 5 nodes in the community
                for node, _ in degrees[:5]:
                    if node != game_appid:  # Don't self-connect
                        G.add_edge(game_appid, node, weight=abs(similarity))
                        edges_added += 1
    
    print(f"[INFO] Added {edges_added} edges connecting games to community members")
    print(f"[INFO] Graph now has {G.number_of_edges()} edges")
    
    return G

def run_louvain_analysis(G: nx.Graph, output_dir: Path) -> Tuple[List[Set[str]], float]:
    """Run Louvain algorithm on the updated graph"""
    
    print(f"[INFO] Running Louvain algorithm on updated graph")
    ...
    try:
        ...
        print(f"[INFO] Using NetworkX Louvain implementation")
    except (ImportError, AttributeError):
        try:
            # Fallback to python-louvain
            import community as community_louvain
            print(f"[INFO] Using python-louvain fallback")
...
def analyze_game_communities(communities: List[Set[str]], game_appids: List[str], 
    ...
    # Save results
    results_path = output_dir / "game_louvain_communities.csv"
    ...
    if games_in_graph > 0:
        community_dist = results_df[results_df['in_graph']]['louvain_community_id'].value_counts()
...
def save_updated_community_assignments(communities: List[Set[str]], output_dir: Path):
    ...
    assignments_path = output_dir / "updated_community_assignments.csv"
...
def main():
    """Main function to add games to graph and run Louvain"""
    
    print("[INFO] Starting process to add games to existing graph and run Louvain")
    ...
    output_dir = Path("yoav/louvain_with_added_games")
    ...
    # Add games to graph
    G_updated = add_games_to_graph(G, games_df, existing_communities)
    
    # Run Louvain algorithm
    communities, modularity = run_louvain_analysis(G_updated, output_dir)
    ...
    print(f"[INFO] Communities detected: {len(communities)}")
